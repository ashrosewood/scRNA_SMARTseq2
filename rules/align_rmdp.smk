...
#	"samples/raw/{sample}_R2.fq.gz"
    output:
        "samples/trimmed/{sample}_R1_trimmed.fq.gz",
#	"samples/trimmed/{sample}_R2_val_2.fq.gz",
        "samples/fastqc/{sample}_R1_trimmed_fastqc.zip",
#	"samples/fastqc/{sample}_R2_val_2_fastqc.zip",
        "samples/fastqc/{sample}_R1_trimmed_fastqc.html",
...
#	"samples/trimmed/{sample}_R2.fq.gz_trimming_report.txt"
    ...
    message:
        """--- Trimming."""
    shell:
        """trim_galore --gzip -o samples/trimmed/ --fastqc_args "--outdir samples/fastqc/" {input[0]}"""
...
#	"samples/trimmed/{sample}_R2_val_2.fq.gz"
    output:
        "samples/fastqscreen/{sample}/{sample}_R1_trimmed_screen.html",
#	"samples/fastqscreen/{sample}/{sample}_R2_val_2_screen.html",
        "samples/fastqscreen/{sample}/{sample}_R1_trimmed_screen.png",
#	"samples/fastqscreen/{sample}/{sample}_R2_val_2_screen.png",
        "samples/fastqscreen/{sample}/{sample}_R1_trimmed_screen.txt",
#	"samples/fastqscreen/{sample}/{sample}_R2_val_2_screen.txt"
    ...
    shell:
        """fastq_screen --aligner bowtie2 --conf {params.conf} --outdir samples/fastqscreen/{wildcards.sample} {input[0]}"""
...
#	"samples/trimmed/{sample}_R2_val_2.fq.gz"
    ...
    shell:
        """
	hisat2 -q -x {params.pathToGenomeIndex} -U {input[0]} -p {threads} --dta --sp 1000,1000 --no-mixed --no-discordant -S samples/hisat2/{wildcards.sample}_output.sam 
